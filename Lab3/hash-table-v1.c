...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	pthread_mutex_init(&hash_table->update_mutex, NULL);
...
{
	// Lock the mutex
	pthread_mutex_lock(&hash_table->update_mutex);
	...
	if (list_entry != NULL) {
		...
		pthread_mutex_unlock(&hash_table->update_mutex);
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	pthread_mutex_destroy(&hash_table->update_mutex);
